...
b_001<-c()
b_0011<-c()
sample_matrix<-matrix(data=c(50,100,50,200),ncol=4,dimnames = list("Test",colnames))
para_00095<-list(N0=200, b=0.0009, l_max=1.45)
para_001<-list(N0=200, b=0.001, l_max=1.5)
para_0011<-list(N0=200, b=0.0011, l_max=1.55)
for(i in 1:500){
  b_0009<-c(b_0009,population_change(sample_matrix,para_00095)-200)
  b_001<-c(b_001,population_change(sample_matrix,para_001)-200)
  b_0011<-c(b_0011,population_change(sample_matrix,para_00105)-200)
}
...
for(name in names(parameter_list)){
  ...
  for( i in 1:20){
    ...
    for(r in 1:runs){
        #ifelse((runif(1)<0.1),
        #N1_new<-ceiling(tail(Patch_1_list$matrix,1)[,"overall"]*runif(n=1, min=0.1, max=0.3)),
        N1_new<-population_change(Patch_1_list$matrix,Patch_1_list$parameters)#)
        ...
        }else{result1<-matrix(data=c(0,0,0,0),ncol=4,dimnames = list(paste("Year_",r,sep=""),colnames))}
        
        if(N2_new>0){
        result2<-matrix(data=Fusion(gamete_vector = gamete_vector2, N=N2_new),ncol=4,dimnames = list(paste("Year_",r,sep=""),colnames))
        gamete_vector2<-gamete_production(disp_result$row2,Patch_2_list$parameters, N2_new)
        }else{result2<-matrix(data=c(0,0,0,0),ncol=4,dimnames = list(paste("Year_",r,sep=""),colnames))}
...
many_plotted_lines(list_differing_rate2$`P2_small_d=0.1_sc=0.05`,upper_y=550,axes=FALSE)
  axis(1, at=c(0,100,200,300,400,500), labels = FALSE,tck = 0.02)
  axis(2,at=c(0,100,200,300,400,500), labels=FALSE,tck = 0.02)
many_plotted_lines(list_differing_rate2$`P2_medium_d=0.1_sc=0.05`,upper_y=550,axes=FALSE)
  axis(1, at=c(0,100,200,300,400,500), labels = FALSE,tck = 0.02)
  axis(2,at=c(0,100,200,300,400,500), labels=FALSE,tck = 0.02)
many_plotted_lines(list_differing_rate2$`P2_large_d=0.1_sc=0.05`,upper_y=550,axes=FALSE)
  axis(1, at=c(0,100,200,300,400,500), labels = FALSE,tck = 0.02)
  axis(2,at=c(0,100,200,300,400,500), labels=FALSE,tck = 0.02)
```
...
many_plotted_lines(to_plot=differing_rates_1, upper_y=550,axes=FALSE)
  axis(1, at=c(0,100,200,300,400,500), labels = FALSE,tck = 0.02)
  axis(2,at=c(0,100,200,300,400,500), labels=FALSE,tck = 0.02)
many_plotted_lines(to_plot=list_of_matrices_1$`P1_d=0.15_sc=0.05_run=10`, upper_y=550,axes=FALSE)
...
many_plotted_lines(to_plot=list_of_matrices_2$`P2_d=0.15_sc=0.05_run=10`, upper_y=550,axes=FALSE)
  axis(1, at=c(0,100,200,300), labels = FALSE,tck = 0.02)
  axis(2,at=c(0,100,200,300,400,500), labels=FALSE,tck = 0.02)
```

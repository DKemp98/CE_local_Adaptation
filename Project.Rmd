...
for (name in names(list_of_matrices_1)){
  ...
  for (r in 1:10){
    list_of_matrices_1[[name]]
  }
    new_cols<-matrix(data=c(rep(d_num,runs+1),rep(sc_num,runs+1), rep(run_num,runs+1),seq(0:runs-1)),ncol=4,nrow=runs+1,byrow=FALSE,dimnames =   list(paste("Year_",seq(0,300,1)),c("dispersal_probability","selection_coefficient","run","Year")))
  one_big_dataframe<-rbind(one_big_dataframe, cbind(d,new_cols))
}
```

```{r}
ggplot(one_big_dataframe)+
  geom_line(aes(x=Year,y=AA, color=as.factor(run)))+
  facet_grid(vars(dispersal_probability), vars(selection_coefficient))+
  scale_color_manual(values=rep(alpha("red",0.2),10))+geom_line(data=one_big_dataframe%>%group_by(interaction(dispersal_probability,selection_coefficient))%>%summarise(mean_AA=mean(AA)), mapping = aes(x = Year, x = mean_AA), color = 'darkred')
```

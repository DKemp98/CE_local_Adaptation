...
for(i in 1:dim(empty)[1]){
  ...
  for (z in to_try){
    ...
    empty[i,k]<-sum(rlogis(50, location=2.5)<test_scale)/125
  }
}
...
return_prob<-function(n,N_max){
      limit<-((1.1*n/N_max)**3)*1.7
      return(sum(rlogis(50, location=2.5)<limit)/125)
}
...
dispersal<-function(matrix1, matrix2, parameter1,parameter2, year, method="default",  ...){
  ...
  disp_prob1<-case_when(method=="even" ~ parameter1$dispersal_prob,
                        method=="population_dependent" ~ return_prob(n1,pop_max1)*parameter1$wanderlust,
                        TRUE ~ 0.1)
  
  disp_prob2<-case_when(method=="even" ~ parameter2$dispersal_prob,
                        method=="population_dependent" ~ return_prob(n2,pop_max2)*parameter2$wanderlust,
                        TRUE ~ 0.1)

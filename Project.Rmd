...
ggplot
```{r}
#Reduced_subset_Patch1_ParaExplo<-subset(one_big_dataframe_1, subset = one_big_dataframe_1[,6]%in% c(0,0.1,0.2,0.3,0.4))
#Reduced_subset_Patch1_ParaExplo<-subset(Reduced_subset_Patch1_ParaExplo, subset=Reduced_subset_Patch1_ParaExplo[,5]%in% c(0.05,0.15,0.25)) %>% data.frame()
ggplot(Reduced_subset_Patch1_ParaExplo,aes(x=Year,y=AA, col=run, group=run))+
  facet_grid(vars(selection_coefficient),vars(dispersal_probability))+
...
```
```{r}
to_plot<-list_of_matrices_1$`P1_d=0.15_sc=0_run=10`
plot(to_plot[,"AA"], col=alpha("red",0.2), lty=1, lwd=1, las=1, type="l", pch=16, xlab="time", ylab="population size (N)", ylim=c(0,450)) # 
colors<-c("red","violet","blue","black")
